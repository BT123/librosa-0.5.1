...
## 在librosa的load函数中，加载的audio file，将会完成采样率以及声道的转换，将采样率转换为22050，声道转换为单声道；
    ...
    """
    # 读取文件之后，将所有frame数据写入list y中
    y = []
    with audioread.audio_open(os.path.realpath(path)) as input_file:
        # 获取audio file 原始的采样率和声道数
        sr_native = input_file.samplerate
        n_channels = input_file.channels
        # 设置读取data的起始位置，offset为指定的秒数，默认为0；
        # 原始采样率是一秒钟采样值的数量，乘秒数再乘声道数，即起始位置跳过前面多少个采样值！
        # np.round() 为返回浮点数四舍五入的值
        # ？？？这样的话，当offset为0时，是否会将格式头部分作为data进行读取？？？ 
    ...
    if y:
        ...
        # 判断大于1，但是当声道数为8000H（32768）时，实际为负数？？？？
        if n_channels > 1:
